...
resource "aws_lambda_function" "send_otp" {
  filename         = "../lambdas/send_otp/send_otp.zip"
  ...
  environment {
    variables = {
      OTP_TABLE = "rwa-voting-otp"
    }
  }
}
resource "aws_lambda_function" "verify_otp" {
  filename      = "../lambdas/verify_otp/verify_otp.zip"
...
resource "aws_lambda_function" "results" {
  filename      = "../lambdas/results/results.zip"
  ...
  environment {
    variables = {
      VOTES_TABLE = "rwa-voting-votes3"
    }
  }
}
resource "aws_apigatewayv2_integration" "send_otp" {
  api_id                 = aws_apigatewayv2_api.api.id
  integration_type       = "AWS_PROXY"
  integration_uri        = aws_lambda_function.send_otp.invoke_arn
...
resource "aws_apigatewayv2_route" "send_otp" {
  ...
  target    = "integrations/${aws_apigatewayv2_integration.send_otp.id}"
}
...
resource "aws_apigatewayv2_route" "results" {
  api_id    = aws_apigatewayv2_api.api.id
  route_key = "GET /results/{electionId}/{postId}"
  target    = "integrations/${aws_apigatewayv2_integration.results.id}"
}
resource "aws_lambda_permission" "send_otp" {
  statement_id  = "AllowAPIGatewaySendOTP"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.send_otp.function_name
  principal     = "apigateway.amazonaws.com"
}
